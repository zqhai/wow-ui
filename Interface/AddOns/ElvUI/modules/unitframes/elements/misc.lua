...
function UF:Construct_AltPowerBar(frame)
	...
	altpower.text:Point("BOTTOM", frame.Health, "TOP")
...
function UF:Construct_FactionIcon(frame)
	local factionIcon = frame:CreateTexture(nil, "OVERLAY")
--	factionIcon:Point('CENTER', frame, 'CENTER')
--	factionIcon:Size(16, 16)
	factionIcon:SetTexCoord(unpack(E.TexCoords))
	factionIcon:SetDrawLayer('OVERLAY', 7)
	...
	for i=1, #buffs do
		if buffs[i].id then
			...
			if name then
				...
				icon:Width(buffs[i].size or db.size);
				icon:Height(buffs[i].size or db.size);
				icon:ClearAllPoints()
