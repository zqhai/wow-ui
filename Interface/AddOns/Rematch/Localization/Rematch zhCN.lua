...
-- Main.lua
	...
	L["Rematch Debug Info"] = "宠物战队插件调试信息"
...
-- Process\QueueProcess.lua
	...
	L["Rematch's leveling queue is empty"] = "宠物战队插件的升级队列是空的"
	L["Now leveling:"] = "正在升级:"
...
-- Menus\TeamMenus.lua
	...
	L["Export this team as a string you can copy elsewhere, such as forums or emails.\n\nThe string format is best for copying multiple teams to be imported together, or for sharing teams with someone in another language.\n\nOther Rematch users can paste this team into their Rematch via Import Team."] = "将队伍导出成字符串，以便复制到其它地方，例如论坛或电子邮件。\n\n字符串格式是一次复制和导入多个队伍，或者与其它语言玩家共享队伍的最佳方式。\n\n已经使用宠物战队插件的玩家可以使用导入队伍的功能，粘贴导入这个队伍"
	L["Import Team"] = "导入队伍"
	L["Import a single team or many teams that was exported from Rematch."] = "导入来自宠物战队插件的一个或多个队伍"
	L["Send Team"] = "发送队伍"
	L["Sharing is disabled in options."] = "选项中的共享功能已经禁用"
	L["Send this team to another online user of Rematch."] = "将这个队伍发送给另一位在线上的宠物战队插件使用者"
	L["Prompt To Load"] = "提示是否加载队伍"
	L["When your new target has a saved team not already loaded, and the target panel isn't on screen, display a popup asking if you want to load the team.\n\nThis is only for the first interaction with a target. You can always load a target's team from the target panel."] = "如果新的目标已经有一个对应的队伍，但是却未加载，而且屏幕上没有显示目标面板时，弹出队伍面板询问是否要加载队伍。\n\n只有第一次和目标互动时才会提示加载。但你总是可以从目标面板加载目标队伍"
	L["With Rematch Window"] = "同时加载宠物战队插件窗口"
	L["Prompt to load with the Rematch window instead of a separate popup dialog."] = "提示并且加载宠物战队插件窗口，而不是只有弹出信息对话框"
	L["Always Prompt"] = "总是提示"
	L["Prompt every time you interact with a target with a saved team not already loaded, instead of only the first time."] = "每次和目标互动，但却没有加载对应的队伍时都提示，而不是只有第一次提示"
	L["When you mouseover a new target that has a saved team not already loaded, immediately load it.\n\nThis is only for the first interaction with a target. You can always load a target's team from the target panel."] = "鼠标指向一个新的目标，但却没有加载对应的队伍时，立即加载。\n\n只有第一次和目标互动时才会提示加载，但你总是可以从目标面板加载目标队伍"
	L["Show After Loading"] = "加载后显示"
	L["After a team auto loads, show the Rematch window."] = "加载队伍后，显示宠物战队插件窗口"
	L["Show On Injured"] = "宠物受伤时显示"
	L["When a team auto loads, show the Rematch window if any pets are injured."] = "自动加载队伍时，如果任何一个宠物受伤的话，显示宠物战队插件窗口"
	...
	L["Import Teams"] = "导入队伍"
	L["Import From Pet Battle Teams"] = "从战斗宠物队伍导入"
	L["Copy your existing teams from Pet Battle Teams to Rematch."] = "将战斗宠物队伍中已有的队伍复制到宠物战队插件"
...
-- Panels\LoadoutPanel.lua
	...
	L["This target has no saved team"] = "该目标没有对应的队伍"
...
-- Panels\OptionPanel.lua
	...
	L["Regardless whether a target's team is already loaded, show the Rematch window when you target something with a saved team."] = "不论是否已经加载目标的队伍，选取的目标有已保存的队伍时，显示宠物战队插件窗口"
	L["Preferred Window Mode"] = "偏好的窗口模式"
	L["Minimized Standalone"] = "小型独立窗口"
	L["When automatically showing the Rematch window, show the minimized standalone window."] = "自动显示宠物战队插件窗口时，显示小型独立窗口"
	L["Maximized Standalone"] = "大型独立窗口"
	L["When automatically showing the Rematch window, show the maximized standalone window."] = "自动显示宠物战队插件窗口时，显示大型独立窗口"
	L["Pet Journal"] = "系统宠物界面"
	L["When automatically showing the Rematch window, show the pet journal."] = "自动显示宠物战队插件窗口时，显示系统宠物界面"
	...
	L["Keep Window On Screen"] = "保持显示窗口"
	...
	L["Don't Minimize With ESC Key"] = "按Esc时不要最小化"
	...
	L["In the ability flyout, show the numbers 1 and 2 to help with the common notation such as \"Pet Name 122\" to know which abilities to use."] = "对于选中的技能，显示数字 1 和 2 以方便识别使用的是哪个技能。例如 \"宠物名字 122\""
	...
	L["Don't hide notes when the ESCape key is pressed or other times it would hide, such as changing tabs or closing Rematch."] = "点击Esc键或其它会隐藏备注的情况下，不要隐藏备注。例如切换标签页或关闭宠物战队插件"
	...
	L["Show the Rematch window after leaving a pet battle."] = "对战结束后显示宠物战队插件窗口"
	...
	L["Place a button on the minimap to toggle Rematch and load favorite teams."] = "在小地图旁放置一个按钮，可供切换显示宠物战队插件和加载偏好的队伍"
	...
	L["Hide the more common tooltips in Rematch."] = "隐藏宠物战队插件的常规鼠标提示"
	...
	L["Turn off Rematch integration with the default pet journal.\n\nYou can still use Rematch in its standalone window, accessed via key binding, /rematch command or from the Minimap button if enabled in options."] = "关闭宠物战队插件，使用系统默认的宠物界面。\n\n仍然可以使用绑定的按键、指令 /rematch 和小地图按钮 (选项中有启用时) 来使用独立窗口的宠物战队插件"
...
-- Widgets\BottomPanel.lua
	...
	L["Disable Rematch"] = "禁用宠物战队插件"
	L["Uncheck this to restore the default pet journal.\n\nYou can still use Rematch in its standlone window, accessed via key binding, /rematch command or from the Minimap button if enabled in options."] = "取消勾选恢复成系统默认的宠物界面，\n\n仍然可以在独立窗口中使用宠物战队插件，使用绑定的按键、/rematch 指令或小地图按钮 (有启用的话) 来开启"
...
-- Dialogs\Share.lua
	...
	L["Sending..."] = "正在发送..."
	L["No response. Lag or they don't have Rematch?"] = "没有回应，网路延迟或对方没有使用宠物战队插件?"
	...
	L["Import"] = "导入"
	L["%s%d\124r Rematch teams have the same name."] = "%s%d\124r 宠物战队插件中已经有相同的队伍名"
	L["Please Wait..."] = "请稍等..."
...
-- Frames\Journal.lua
	L["Enable Rematch"] = "启用宠物战队插件"
	L["Check this to use Rematch in the pet journal."] = "启用这个选项，在系统默认的宠物界面中使用宠物战队插件"
