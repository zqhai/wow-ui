...
ChatFilter:SetScript("OnEvent", function(self, event)
	if (not E.global.chatfilter.Enabled) then return end
	...
	elseif (event == "PLAYER_ENTERING_WORLD") then
 		if (Config.FilterByLevel) then
			for k, v in pairs(E.global.chatfilter.ShieldPlayers) do
				_ShieldTable[v] = true
			end
...
local function ChatFilter_Rubbish(self, event, msg, player, _, _, _, flag, _, _, channel, _, lineId, guid)
	...
	if channel == '友团组团通信' then return end
	if (lineId ~= prevLineId) then
		...
		if (Config.FilterAdvertising) then
			for i = 1, getn(E.global.chatfilter.SafeWords) do
				if (msg:find(E.global.chatfilter.SafeWords[i])) then
					matchs = matchs - 1
				end
			end
