...
if (GetLocale() == "zhCN") then
	...
	BigFootBank_SayTitle="<EUI黑市助手通知> 已经更新黑市拍卖行数据"
	BigFootBank_LogoutSayTitle ="<EUI黑市助手通知> 您的黑市拍卖行有物品:%s 剩余时间为：%s";

	BigFootBank_MINIMAP_TOOLTIP1="EUI黑市助手"
	BigFootBank_MINIMAP_TOOLTIP2="点击打开黑市界面"

	BigFootBank_OffLineTime="<EUI黑市助手> 黑市数据更新时间："
	BigFootBank_OffLineNoData="<EUI黑市助手> 沒有黑市数据。"
elseif (GetLocale() == "zhTW") then
	BigFootBank_BFchannel="EUI世界頻道";
	BigFootBank_SayTitle="<EUI黑市助手通知> 已經更新黑市拍賣行數據"
	BigFootBank_LogoutSayTitle="<EUI黑市助手通知> 您的黑市拍賣行有物品:%s 剩餘時間為：%s"

	BigFootBank_MINIMAP_TOOLTIP1="EUI黑市助手"
	BigFootBank_MINIMAP_TOOLTIP2="点击打开黑市界面"

	BigFootBank_OffLineTime="<EUI黑市助手> 黑市數據更新時間："
	BigFootBank_OffLineNoData="<EUI黑市助手> 沒有黑市數據。"
else
...
local function BigFootBank_SendData(sender)
	...
	if BigFootBankSaveData[realmName].bankData then
		if BigFootBankSaveData[realmName].bankData.numItems and BigFootBankSaveData[realmName].bankData.numItems>0 then
			Str = Str..BigFootBankSaveData[realmName].bankData.time.."#"..E:GetServerName().."#"..BigFootBankSaveData[realmName].bankData.numItems.."#";
	...
	if completely then
		for k,v in pairs (SayTab) do
			...
			if sender then
				SendAddonMessage("BigFootBankData",v,'WHISPER',sender)
				-- print("send",v)
			else
				if IsInGroup() then
				...
				if IsInGuild() then
					SendAddonMessage("BigFootBankData",v,"GUILD");
				end
				-- print("send",v)
			end
		end
...
local function BigFootBank_CheckResiveData(reseverStr,sender)
	...
	if reseverStr and reseverStr~="" then
		...
		if itemnumber then
			...
			local Bok =true
			if itemServer ~=E:GetServerName() then
				Bok =false
			end
...
function BigFootBank_OffLineUpdata(link, text, button)
	...
	if not done then
		E:ScheduleTimer(BigFootBank_OffLineUpdata, 0.2); -- 延迟调用一下。因为 如果本来官方UI没有载入 第一会被官方界面刷新掉。补一次
		done =true
	end
end
